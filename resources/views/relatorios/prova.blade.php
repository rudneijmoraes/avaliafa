...
@section('title', 'Relatório da Prova — AvaliaFA')
@section('page-title', 'Relatório da Prova')
...
<div class="page-header">
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
        <div>
            ...
            <p class="page-subtitle">Sistema: {{ $exam->clientSystem?->name ?? '—' }} · Status: {{ ucfirst($exam->status) }}</p>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button type="button" class="btn btn-secondary" onclick="document.getElementById('export-panel').style.display=document.getElementById('export-panel').style.display==='none'?'block':'none'">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" style="margin-right:5px"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                Exportar CSV
            </button>
...
<div id="export-panel" style="display:none;margin-bottom:16px">
    <div class="card">
        <div style="padding:14px 20px;border-bottom:1px solid var(--surface-border);font-weight:700;color:var(--text-primary);display:flex;align-items:center;gap:8px">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
            Configurar exportação CSV
        </div>
        <form method="GET" action="{{ route('relatorios.prova.export', $exam) }}">
            <input type="hidden" name="status" value="{{ request('status') }}">
            <input type="hidden" name="resultado" value="{{ request('resultado') }}">
            <input type="hidden" name="busca" value="{{ request('busca') }}">
            <input type="hidden" name="incluir_simulados" value="{{ $includeSimulations ? '1' : '0' }}">
            <div style="padding:16px 20px">
                ...
                <div style="display:flex;gap:8px;align-items:center">
                    <button type="submit" class="btn btn-primary btn-sm">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                        Baixar CSV
                    </button>
...
<div class="card" style="margin-bottom:16px">
    <div style="padding:14px 20px">
        <form method="GET" action="{{ route('relatorios.prova', $exam) }}" style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end">
            ...
            <label style="display:flex;align-items:center;gap:8px;padding:8px 10px;height:36px;border:1px solid var(--surface-border);border-radius:10px;background:var(--surface-bg);cursor:pointer">
                <input type="hidden" name="incluir_simulados" value="0">
                <input type="checkbox" name="incluir_simulados" value="1" {{ $includeSimulations ? 'checked' : '' }}>
                <span style="font-size:0.8rem;color:var(--text-primary)">Incluir simulados</span>
            </label>
            <button type="submit" class="btn btn-secondary btn-sm" style="height:36px">Filtrar</button>
            @if(request()->hasAny(['busca', 'status', 'resultado', 'incluir_simulados']))
            <a href="{{ route('relatorios.prova', $exam) }}" class="btn btn-ghost btn-sm" style="height:36px">Limpar</a>
            @endif
        </form>
    </div>
</div>
...
<div class="alert alert-warning" style="margin-bottom:16px">
    O relatorio mostra apenas sessoes oficiais por padrao. Marque <strong>Incluir simulados</strong> para localizar sessoes de teste.
</div>
@endif
    ...
    <div style="overflow-x:auto">
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <div style="font-size:0.75rem;color:var(--text-muted)">
                            ...
                            <span class="badge badge-warning" style="margin-left:6px">Simulado</span>
                            @endif
                        </div>
                    </td>
                    ...
                    <td>
                        ...
                        <form method="POST" action="{{ route('relatorios.prova.grade.update', [$exam, $session]) }}" style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
                            @csrf
                            @method('PATCH')
                            <input type="number" name="final_score" min="0" max="10" step="0.01"
                                   value="{{ $session->final_score !== null ? number_format((float) $session->final_score, 2, '.', '') : '' }}"
                                   class="input" style="width:90px;height:30px;padding:6px 8px">
                            <button type="submit" class="btn btn-secondary btn-sm" style="height:30px;padding:0 10px">Salvar</button>
                ...
                @empty
                <tr>
                    <td colspan="10" style="text-align:center;padding:48px 16px;color:var(--text-muted)">
